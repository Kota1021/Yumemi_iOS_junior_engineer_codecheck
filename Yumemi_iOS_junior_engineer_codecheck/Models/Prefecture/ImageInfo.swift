...
struct ImageInfoSets{
    ...
    static public func items(of prefecture: String) -> [PrefectureImageInfo]{ data.filter{$0.pref == prefecture } }
    
    static public func thumbnailURL(of prefecture: String) -> URL{
        let urls = items(of:prefecture).map{ $0.urlThumb }
        guard let url = urls.first else { fatalError("ImageInfoSets: image URL info not found. of \(prefecture)") }
        return url
    }
